...
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-w -s" -a -installsuffix cgo -o static-kanban-client .
...
ENV SERVICE=:static-kanban-client \
    POSITION=Runtime \
...
COPY --from=builder /go/src/bitbucket.org/latonaio/static-kanban-client .

CMD ["./static-kanban-client"]
